...
function dataTableFunctions() {
    ...
    let a = document.querySelector('#myTableNajemnici > div > table > tbody > tr');
    ...
    fetch("http://localhost:5173/budovyData.json")
        ...
        .then((data) => {
            data.map((item, key) => {
                $(a).append(`
                    <tr class="table__row row-${key+1}">
                        <td class="table__name_najemnika">Petr</td>
                        <td class="table__budova_najemnika">Budova 1</td>
                        <td class="table__pokoj_najemnika">104</td>
                        <td><button class="table__edit">Edit</button></td>
                    </tr>
                `)
            });
